...
FROM r_flux_indicators{table_suffix} fi
INNER JOIN r_pool_collection_pools{table_suffix} p_src
    ON fi.source_pool_collection_id = p_src.pool_collection_id
INNER JOIN r_pool_collection_pools{table_suffix} p_dst
    ON fi.sink_pool_collection_id = p_dst.pool_collection_id
INNER JOIN fluxes{table_suffix} f
...
INNER JOIN r_change_type_categories{table_suffix} ct
    ON fi.change_type_category_id = ct.id
LEFT JOIN disturbancedimension{table_suffix} di
    ON f.disturbancedimid = di.id
LEFT JOIN disturbancetypedimension{table_suffix} dt
